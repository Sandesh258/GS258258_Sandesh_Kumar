...
const Chart = () => {
  ...
  useEffect(() => {
    ...
    const generateRandomData = () => {
      const storeSalesData: ChartData[] = Array.from({ length: 3 }, (_, i) => {
        const weekNumber = i + 1;
        
        
        const salesUnitsBySKU = skus.reduce((acc, sku) => {
          acc[sku.id] = getRandomInt(10, 50); 
          return acc;
        ...
        const totalSalesDollars = skus.reduce((total, sku) => {
          ...
          const variablePrice = sku.price * (getRandomInt(90, 110) / 100); 
          return total + (salesUnits * variablePrice);
        }, 0);

        const totalGmDollars = skus.reduce((total, sku) => {
          const salesUnits = salesUnitsBySKU[sku.id];
          const variableCost = sku.cost * (getRandomInt(90, 110) / 100); 
        ...
        return {
          ...
          totalSalesUnits: Object.values(salesUnitsBySKU).reduce((acc, units) => acc + units, 0) 
        };
      });
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          ...
          <Tooltip />
          <Legend />
          <Bar dataKey="totalSalesUnits" fill="#8884d8" name="Total Sales Units" />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
